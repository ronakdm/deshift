...
class GroupSpectralRiskTest(parameterized.TestCase):
  ...
  def test_spectral_risk(self, penalty):
    ...
    shift_cost = 1.0
    max_oracle = make_group_spectral_risk_measure(spectrum, penalty, shift_cost)
    ...
    weights_indiv = [
        elem.item() for elem in spectral_risk_measure_maximization_oracle(spectrum, 1.0, penalty, losses.numpy())
    ]
    ...
    shift_cost = 0.0
    spectrum = make_superquantile_spectrum(2, 0.5)
    max_oracle = make_group_spectral_risk_measure(spectrum, penalty, shift_cost)
